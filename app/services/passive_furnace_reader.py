...
class PassiveFurnaceReader:
    ...
    def connect(self) -> bool:
        ...
        try:
            self._serial = serial.Serial(
                ...
                stopbits=stopbits_map[float(self.config.stopbits)],
                timeout=max(0.05, float(self.config.timeout)),
            )
    ...
    def read_temperatures(self) -> tuple[float | None, float | None]:
        if self.test_mode:
            ...
            pv = round(min(650.0, 25.0 + elapsed * 2.4) + math.sin(elapsed / 6.0) * 3.0, 2)
            sv = 650.0 if elapsed >= 30 else 400.0
            return pv, sv
