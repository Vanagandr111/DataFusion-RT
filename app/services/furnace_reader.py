...
class FurnaceReader:
    ...
    def _read_register(self, register: int) -> int | None:
        ...
        try:
            response = self._read_holding_register(register)
        except Exception:
            self.logger.warning(
    ...
    def _read_holding_register(self, register: int):
        if self._client is None:
            return None
    ...
    def _resolve_device_id_arg_name(self) -> str | None:
        ...
        for candidate in ("device_id", "slave", "unit"):
            if candidate in params:
                self._device_id_arg_name = candidate
                return candidate

        self._device_id_arg_name = None
        return None
