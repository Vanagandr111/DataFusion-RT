...
def probe_scale_port(
    ...
    *,
    ...
    if test_mode:
        return True, "Тестовый режим: виртуальные весы доступны."

    port_name = config.port.strip()
    if not port_name:
        logger.warning("Проверка весов пропущена: COM-порт не выбран.")
        return False, "COM-порт для весов не выбран."
        ...
        for _ in range(3):
            ...
            if text:
                ...
                if parsed is not None:
                    return True, f"Весы обнаружены на {port_name}: {parsed:.3f} г"
...
def probe_furnace_port(
    ...
    *,
    ...
    if test_mode:
        return True, "Тестовый режим: виртуальная печь доступна."
    ...
    try:
        pv = reader.read_pv()
        if pv is not None:
            return True, f"Печь обнаружена на {port_name}: текущая температура {pv:.2f} °C"

        sv = reader.read_sv()
        if sv is not None:
            return True, f"Печь обнаружена на {port_name}: заданная температура {sv:.2f} °C"

        logger.warning("Нет Modbus-ответа от %s во время проверки печи.", port_name)
        return False, f"Нет ответа Modbus от {port_name}. Проверьте адрес устройства и регистры."
    finally:
        reader.close()
